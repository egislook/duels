...
var dbOpts = {
    ...
    test : 'mongodb://good:goodreadandwrite@ds033599.mongolab.com:33599/test_league'
}
...
function get(app, colName, callback, query, limit){
    query = query || {};
    var db = app.cache.db;
    col(db, colName, function(collection){
        if(!limit){
            ...
            collection.find(query).limit(limit).toArray(function(e, data){
                callback(data);
            });
        }
    })
